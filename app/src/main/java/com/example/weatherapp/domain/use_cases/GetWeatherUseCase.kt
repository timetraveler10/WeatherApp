...
class GetWeatherUseCase @Inject constructor(
        ...
        return flow {
            ...
            try {
                ...
                val lngLatStr = datastoreRepo.readLngLat().flowOn(Dispatchers.Default).catch {
                    emit(Resource.Error<Weather>(message = it.message))
                }.first()


                val currentWeather = client.get<CurrentWeatherDto> {
                    url(CURRENT_END_POINT)
                    parameter(key = "location", value = lngLatStr)
                }

                val hourlyWeather = client.get<HourlyForecastDto> {
                    url(HOURLY_WEATHER_END_POINT)
                    parameter(key = "location", value = lngLatStr)
                }
                ...
                emit(
                    Resource.Success(
                        data = Weather(
                            current = currentWeather.current.toCurrentWeather(),
                            hourlyWeather = hourlyWeather.forecast.map { it.toHourlyWeather() },
                            locationEntity = spiltStringLngLat(lngLatStr)),
                    )
                )
    ...
    private fun spiltStringLngLat(lngLat: String): LocationEntity {
        val list = lngLat.split(',').map { it.toDouble() }
        return LocationEntity(lng = list[0], lat = list[1])
    }
}